...
Fetch inline review comments (the ones attached to specific file lines). Use
`--paginate` and project just the fields you need so the payload stays small and
parseable:

```bash
gh api repos/{owner}/{repo}/pulls/{number}/comments --paginate \
  --jq '.[] | {id, in_reply_to_id, path, line, original_line, start_line, original_start_line, side, start_side, diff_hunk, body, user: .user.login}'
```

Fetch general PR discussion comments (not attached to code). This endpoint
paginates too — **always pass `--paginate`** or comments past the first page are
silently dropped:
...
**Group the inline comments into threads correctly.** A thread root has no
`in_reply_to_id`; each reply carries `in_reply_to_id` pointing at the root
comment's `id`. Group by chaining every reply back to its root via
`in_reply_to_id` — **do not** key on `path`/`line`/`position` (those are display
metadata, and `position` is `null` for outdated comments, which would scatter
threads). Use `path` + `line`/`original_line` only for displaying where each
thread lives.

Summarize what you found: thread count (grouped, counting roots — not raw
comments), files touched, and how many general comments exist. Wait for the user
to confirm before proceeding.

**Resolved threads:** the REST endpoint above carries no resolved/unresolved
state. If the user wants to skip resolved threads or filter to unresolved-only,
fetch resolution state via GraphQL and join on comment id:
...
Map each thread's root `databaseId` to its `isResolved` flag. If you cannot or
do not run this query, say so plainly — do not claim a thread is resolved/
unresolved without the data.

**Cap:** if there are more than 100 grouped threads, say so and offer to filter
(by file, by reviewer, or unresolved-only — the last requires the GraphQL query
...
1. **Present the thread.** Header with file path and the comment's line range
   (`start_line`–`line` when it spans multiple lines, otherwise the single
   `line`), then the reviewer comment(s) in chronological order.
2. **Read the real code.** Open the file and show the cited line(s) with about
   ...
   proposing anything. The branch may have moved since the comment was posted, so
   anchor reliably:
   - The comment's `diff_hunk` is the exact code the reviewer saw — use it (plus
     `original_line` / `original_start_line`) to relocate the intended code by
     content rather than trusting the raw line number.
   - If the current line numbers no longer match that content, **warn that the
     line numbers may be stale** and proceed against the relocated code.
   - If the file no longer exists (deleted or renamed since the comment), say so
     and treat the thread as discussion-only unless the user directs otherwise.
3. **Propose a fix.** Categorize the thread as an **actionable change**, a
   **discussion point**, or **ambiguous**. For actionable changes, describe the
   specific edit you'd make. If a comment contains a ```suggestion``` block, that
   block is the reviewer's exact proposed replacement — apply it verbatim (still
   gated) unless it's plainly wrong.
...
editing — the strong model thinks, scopes, and QAs while a lighter model does the
edits.

### 3'a — Triage into a fix plan

Read the real code behind each thread first (don't plan from the comment text
alone) — same anchoring rules as interactive Step 3.2: use `diff_hunk` /
`original_line` to relocate moved code, and treat a ```suggestion``` block as the
reviewer's exact proposed edit. Then sort every thread into one of:
